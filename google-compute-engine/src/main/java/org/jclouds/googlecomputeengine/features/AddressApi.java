...
public interface AddressApi {
   ...
   /**
    * @see org.jclouds.googlecomputeengine.features.AddressApi#aggregatedListAtMarker(String, org.jclouds.googlecomputeengine.options.ListOptions)
    */
   @Named("Addresss:aggregatedList")
   @GET
   @Consumes(MediaType.APPLICATION_JSON)
   @Path("/aggregated/addresses")
   @OAuthScopes(COMPUTE_READONLY_SCOPE)
   @ResponseParser(ParseAddresses.class)
   @Fallback(EmptyIterableWithMarkerOnNotFoundOr404.class)
   ListPage<Address> aggregatedListFirstPage();
   ...
   @ResponseParser(ParseAddresses.class)
   @Fallback(EmptyIterableWithMarkerOnNotFoundOr404.class)
   ListPage<Address> aggregatedListAtMarker(@QueryParam("pageToken") @Nullable String marker);

   /**
    * TODO live and expect tests for aggregatedList*
    * Retrieves the listPage of address resources contained within the specified project.
    * By default the listPage as a maximum size of 100, if no options are provided or ListOptions#getMaxResults() has
    * not been set.
    *
    * @param marker      marks the beginning of the next list page
   ...
   @ResponseParser(ParseAddresses.class)
   @Fallback(EmptyIterableWithMarkerOnNotFoundOr404.class)
   ListPage<Address> aggregatedListAtMarker(@QueryParam("pageToken") @Nullable String marker, ListOptions listOptions);

   /**
    * A paged version of AddressApi#aggregatedListPage()
    *
    * @return a Paged, Fluent Iterable that is able to fetch additional pages when required
    * @see org.jclouds.collect.PagedIterable
    * @see org.jclouds.googlecomputeengine.features.AddressApi#aggregatedListAtMarker(String, org.jclouds.googlecomputeengine.options.ListOptions)
   ...
   @Transform(ParseAddresses.ToPagedIterable.class)
   @Fallback(EmptyPagedIterableOnNotFoundOr404.class)
   PagedIterable<Address> aggregatedList();
   ...
   @Transform(ParseAddresses.ToPagedIterable.class)
   @Fallback(EmptyPagedIterableOnNotFoundOr404.class)
   PagedIterable<Address> aggregatedList(ListOptions options);
